/**
 * Content Moderation Editor Plugin - Complete Design Editing Configuration for CE.SDK
 *
 * This plugin provides a production-ready content moderation editor configuration with
 * templates, elements, asset libraries, and comprehensive editing capabilities.
 *
 * @example Basic usage
 * ```typescript
 * import CreativeEditorSDK from '@cesdk/cesdk-js';
 * import { ContentModerationEditorConfig } from './plugin';
 *
 * const cesdk = await CreativeEditorSDK.create('#editor', config);
 * await cesdk.addPlugin(new ContentModerationEditorConfig());
...
/**
 * Content Moderation Editor configuration plugin.
 *
 ...
 * @public
 */
export class ContentModerationEditorConfig implements EditorPlugin {
  /**
   * Unique identifier for this plugin.
   * Used to identify the plugin in the CE.SDK plugin registry.
   */
  name = 'cesdk-content-moderation-editor';
  ...
  /**
   * Initialize the content moderation editor configuration.
   *
  ...
  async initialize({ cesdk, engine }: EditorPluginContext) {
    if (cesdk) {
      ...
      // This ensures a clean slate when applying the content moderation editor config
      cesdk.resetEditor();
      // #endregion
